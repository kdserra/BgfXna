...
public class AutoPongGame : Game
{
    ...
    public Point GameBounds { get; } = new Point(1280, 720); //window resolution
    ...
    public int PointsRight { get; private set; }
    ...
    public AutoPongGame()
    {
        ...
        _graphics.PreferredBackend = SamplePlatform.Backend;
        _graphics.PreferredBackBufferWidth = GameBounds.X;
        _graphics.PreferredBackBufferHeight = GameBounds.Y;
        IsMouseVisible = true;
    }

    public void SetNativeWindowHandle(IntPtr windowHandle)
    {
    ...
    public void SetNativeWindowHandleType(NativeWindowHandleKind kind)
    {
        _graphics.NativeWindowHandleKind = kind;
    }
    ...
    protected override void Update(GameTime gameTime)
    {
        ...
        {
            SoundFX.PlayWave(440.0f, 100, WaveType.Sin, 0.2f);
        }
        else if (JingleCounter == speed * 2)
        {
            SoundFX.PlayWave(523.25f, 100, WaveType.Sin, 0.2f);
        }
        else if (JingleCounter == speed * 3)
        {
            SoundFX.PlayWave(659.25f, 100, WaveType.Sin, 0.2f);
        }
        else if (JingleCounter == speed * 4)
        {
            SoundFX.PlayWave(783.99f, 100, WaveType.Sin, 0.2f);
        }
    ...
    protected override void Draw(GameTime gameTime)
    {
        ...
        for (int i = 0; i < total; i++)
        {
            DrawRectangle(
                _spriteBatch,
                new Rectangle(GameBounds.X / 2 - 4, 5 + (i * 20), 8, 8),
                Color.White * 0.2f
            );
        }
        ...
        for (int i = 0; i < PointsLeft; i++)
        {
            DrawRectangle(
                _spriteBatch,
                new Rectangle((GameBounds.X / 2 - 25) - i * 12, 10, 10, 10),
                Color.White * 1.0f
            );
        }
        for (int i = 0; i < PointsRight; i++)
        {
            DrawRectangle(
                _spriteBatch,
                new Rectangle((GameBounds.X / 2 + 15) + i * 12, 10, 10, 10),
                Color.White * 1.0f
            );
        }
...
{
    public void PlayWave(float frequency, int durationMs, WaveType waveType, float volume)
    {
    }
}
